...
<div class="space-y-6">
    <div class="flex items-center justify-between gap-3 mb-2">
        <div>
            <h1 class="font-display text-3xl text-slate-900">{{ __('System overview') }}</h1>
            <p class="text-sm text-slate-500">{{ __('Welcome back to BookMe administration.') }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="card flex flex-col justify-between p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400">{{ __('Total') }}</span>
            </div>
            <div>
                <p class="text-3xl font-bold text-slate-900">{{ $stats['total_profiles'] }}</p>
                <p class="text-sm text-slate-500 font-medium">{{ __('Registered businesses') }}</p>
            </div>
        </div>
        ...
        <div class="card flex flex-col justify-between p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400">{{ __('Active') }}</span>
            </div>
            <div>
                <p class="text-3xl font-bold text-slate-900">{{ $stats['active_profiles'] }}</p>
                <p class="text-sm text-slate-500 font-medium">{{ __('Published profiles') }}</p>
            </div>
        </div>
        ...
        <div class="card flex flex-col justify-between p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400">{{ __('Activity') }}</span>
            </div>
            <div>
                <p class="text-3xl font-bold text-slate-900">{{ $stats['total_appointments'] }}</p>
                <p class="text-sm text-slate-500 font-medium">{{ __('All appointments') }}</p>
            </div>
        </div>
        ...
        <div class="card flex flex-col justify-between p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400">{{ __('Revenue') }}</span>
            </div>
            <div>
                <p class="text-3xl font-bold text-slate-900">€{{ number_format($stats['total_revenue'], 2) }}</p>
                <p class="text-sm text-slate-500 font-medium">{{ __('Confirmed orders') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="card overflow-hidden !p-0">
                <div class="px-6 py-4 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                    <h2 class="font-bold text-slate-900">{{ __('Latest businesses') }}</h2>
                    <a href="{{ route('admin.profiles') }}" class="text-xs font-bold text-emerald-600 hover:text-emerald-700 transition">{{ __('Show all') }}</a>
                </div>
            ...
            <div class="card overflow-hidden !p-0">
                <div class="px-6 py-4 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                    <h2 class="font-bold text-slate-900">{{ __('Upcoming appointments') }}</h2>
                    <a href="{{ route('admin.appointments') }}" class="text-xs font-bold text-emerald-600 hover:text-emerald-700 transition">{{ __('Calendar') }}</a>
                </div>
                <div class="divide-y divide-slate-100">
                        ...
                        <div class="px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 hover:bg-slate-50/50 transition-colors">
                            ...
                            <div class="flex items-center gap-6">
                                <div class="text-right">
                                    ...
                                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{{ __('Start') }}</p>
                                </div>
                    ...
                    @empty
                        <div class="p-12 text-center">
                            <p class="text-sm text-slate-500 italic">{{ __('No appointments in the system yet.') }}</p>
                        </div>
                    @endforelse
                </div>
            </div>
        ...
        <div class="space-y-6">
            <!-- Quick Actions -->
            <div class="card p-6">
                <h2 class="font-bold text-slate-900 mb-4">{{ __('Quick actions') }}</h2>
                <div class="grid grid-cols-1 gap-2">
                    <a href="{{ route('admin.profiles') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl bg-slate-50 hover:bg-emerald-50 text-slate-700 hover:text-emerald-700 transition-all group">
                        ...
                        <span class="text-sm font-bold">{{ __('Add business') }}</span>
                    </a>
                    <a href="{{ route('admin.invoices') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl bg-slate-50 hover:bg-emerald-50 text-slate-700 hover:text-emerald-700 transition-all group">
                        ...
                        <span class="text-sm font-bold">{{ __('Issue invoice') }}</span>
                    </a>
                </div>
            </div>
            ...
            @if($expiring_trials->isNotEmpty())
                <div class="card p-6 border-orange-100 bg-orange-50/20">
                    <div class="flex items-center gap-3 mb-4 text-orange-700">
                        ...
                        <h2 class="font-bold">{{ __('Expiring trial periods') }}</h2>
                    </div>
                    <div class="space-y-3">
                        @foreach($expiring_trials as $profile)
                            <div class="p-4 bg-white rounded-xl border border-orange-100 shadow-sm hover:shadow-md transition-shadow">
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    @else
                                        <span class="text-[10px] font-bold text-rose-600 bg-rose-50 px-2 py-0.5 rounded-md">{{ __('Expired') }}</span>
                                    @endif
                                </div>
                            </div>
                        @endforeach
