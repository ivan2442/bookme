                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.invoices.preview', $invoice) }}" target="_blank" class="p-1.5 rounded-lg bg-slate-100 text-slate-600 hover:bg-blue-50 hover:text-blue-600 transition" title="Náhľad">
                                    ...
                                    <form action="{{ route('admin.invoices.send', $invoice) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="p-1.5 rounded-lg bg-slate-100 text-slate-600 hover:bg-emerald-50 hover:text-emerald-600 transition" title="Odoslať emailom">
                                    ...
                                    @if($invoice->status === 'unpaid')
                                        <form action="{{ route('admin.invoices.status.update', $invoice) }}" method="POST" class="inline">
                                            ...
                                            <button type="submit" class="p-1.5 rounded-lg bg-emerald-100 text-emerald-700 hover:bg-emerald-200 transition" title="Označiť ako uhradené">
                                    ...
                                    <form action="{{ route('admin.invoices.delete', $invoice) }}" method="POST" class="inline" onsubmit="return confirm('Naozaj chcete odstrániť túto faktúru?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-1.5 rounded-lg bg-slate-100 text-slate-400 hover:bg-rose-50 hover:text-rose-600 transition" title="Odstrániť">
