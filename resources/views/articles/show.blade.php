...
<div class="py-12 max-w-3xl mx-auto">
    ...
    <header class="mb-8">
        ...
        @if($article->author)
            <p class="text-slate-500 text-sm italic">{{ __('Author') }}: {{ $article->author->name }}</p>
        @endif
    </header>
    ...
    <footer class="mt-16 pt-8 border-t border-slate-100">
        <div class="bg-emerald-50 rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
                <h3 class="text-lg font-bold text-slate-900 mb-2">{{ __('Want to make your business visible?') }}</h3>
                <p class="text-slate-600 text-sm">{{ __('Register on BookMe and get more customers today.') }}</p>
            </div>
            <a href="{{ route('auth.login') }}" class="px-6 py-3 bg-emerald-600 text-white font-bold rounded-xl shadow-lg shadow-emerald-200/60 hover:bg-emerald-700 transition">{{ __('Add business') }}</a>
        </div>
    </footer>
</div>
@endsection
