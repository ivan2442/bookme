...
<div class="space-y-6">
    <div class="flex items-center justify-between gap-3 mb-2">
        <div>
            <h1 class="font-display text-3xl text-slate-900">{{ __('My team') }}</h1>
            <p class="text-sm text-slate-500">{{ __('Manage employees and their assignment to the business.') }}</p>
        </div>
    ...
    <div class="grid lg:grid-cols-[1fr,350px] gap-6 items-start">
        <div class="card space-y-3">
            <h2 class="font-semibold text-lg text-slate-900">{{ __('Current team') }}</h2>
            <div class="space-y-3 max-h-[75vh] overflow-y-auto pr-1">
                @forelse($employees as $employee)
                    <div class="border border-slate-100 rounded-xl p-4 bg-white/80 space-y-3 shadow-sm hover:border-emerald-100 transition-colors">
                        ...
                        <details class="text-sm">
                            <summary class="cursor-pointer text-slate-600 hover:text-slate-900 font-medium py-1">{{ __('Edit details') }}</summary>
                            <form method="POST" action="{{ route('owner.employees.update', $employee) }}" class="space-y-3 mt-2 pt-2 border-t border-slate-100">
                                @csrf
                                <div>
                                    <label class="text-[10px] uppercase font-bold text-slate-500">{{ __('Name') }}</label>
                                    <input type="text" name="name" class="input-control !py-1.5 !text-sm" value="{{ $employee->name }}" required>
                                </div>
                                <div class="grid grid-cols-2 gap-2">
                                    <div>
                                        <label class="text-[10px] uppercase font-bold text-slate-500">{{ __('Email') }}</label>
                                        <input type="email" name="email" class="input-control !py-1.5 !text-sm" value="{{ $employee->email }}">
                                    </div>
                                    <div>
                                        <label class="text-[10px] uppercase font-bold text-slate-500">{{ __('Phone') }}</label>
                ...
                @empty
                    <p class="text-sm text-slate-500 italic p-8 text-center">{{ __('You have no employees in your team yet.') }}</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="card p-6">
                ...
                <p class="text-sm text-slate-600 leading-relaxed">
                    {{ __('Employees are a key part of your reservation system. You can assign specific services to each team member in the Services section.') }}
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 overflow-hidden">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-xl font-display font-semibold text-slate-900">{{ __('Add team member') }}</h3>
            ...
            <form method="POST" action="{{ route('owner.employees.store') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" class="input-control" placeholder="{{ __('e.g. John Doe') }}" required>
                </div>
                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        <label class="label">{{ __('Email') }}</label>
                        <input type="email" name="email" class="input-control" placeholder="john@doe.com">
                    </div>
                    <div>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-slate-50">
                    <button type="button" onclick="closeAddEmployeeModal()" class="px-4 py-2 text-sm font-semibold text-slate-600 hover:text-slate-800 transition">{{ __('Cancel') }}</button>
                    <button type="submit" class="px-6 py-2 rounded-xl bg-slate-900 text-white font-semibold hover:bg-slate-800 transition shadow-lg shadow-slate-200/50">{{ __('Save employee') }}</button>
                </div>
            </form>
        </div>
    </div>
