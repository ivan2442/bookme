...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
        <div>
            <h1 class="font-display text-3xl text-slate-900">{{ __('Revenue and performance') }}</h1>
            <p class="text-sm text-slate-500">{{ __('Overview of the financial performance of your business.') }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="card flex flex-col justify-between p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400">{{ __('Completed') }}</span>
            </div>
            <div>
                <p class="text-3xl font-bold text-slate-900">{{ number_format($stats['count']) }}</p>
                <p class="text-sm text-slate-500 font-medium">{{ __('Appointments') }} ({{ $months[$selectedMonth] }})</p>
            </div>
        </div>
        ...
        <div class="card flex flex-col justify-between p-6">
            ...
            <div>
                ...
                <p class="text-sm text-slate-500 font-medium">{{ __('Total revenue') }}</p>
            </div>
        </div>
        ...
        <div class="card flex flex-col justify-between p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400">{{ __('Performance') }}</span>
            </div>
            <div>
                <p class="text-3xl font-bold text-slate-900">{{ number_format($stats['hours'], 1) }}h</p>
                <p class="text-sm text-slate-500 font-medium">{{ __('Worked hours') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="card p-6">
        <h3 class="font-bold text-slate-900 mb-6 flex items-center gap-2">
            ...
            {{ __('Revenue development for the last 12 months') }}
        </h3>
    ...
    <div class="card overflow-hidden !p-0">
        <div class="px-6 py-4 border-b border-slate-50 bg-slate-50/50">
            <h3 class="font-bold text-slate-900">{{ __('History of completed reservations') }}</h3>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                <thead>
                    <tr class="bg-slate-50 text-slate-500 text-[10px] uppercase tracking-widest font-bold">
                        <th class="px-6 py-4">{{ __('Date and time') }}</th>
                ...
                <tbody class="divide-y divide-slate-100 bg-white/50">
                    @forelse($latestPayments as $appointment)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <span class="font-medium text-slate-900">{{ $appointment->metadata['service_name_manual'] ?? ($appointment->service?->name ?? __('Manual service')) }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-[10px] px-2 py-0.5 rounded-md bg-slate-100 text-slate-500 font-bold uppercase tracking-tight">
                                    {{ optional($appointment->employee)->name ?? __('No employee') }}
                                </span>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-slate-500 italic">
                                {{ __('No completed reservations found.') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
