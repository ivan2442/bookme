...
<div class="space-y-6">
    <div class="flex items-center justify-between gap-3 mb-2">
        <div>
            <h1 class="font-display text-3xl text-slate-900">Všetky rezervácie</h1>
            <p class="text-sm text-slate-500">Kompletný prehľad termínov vašej prevádzky.</p>
        </div>
    </div>
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="bg-slate-50 text-left text-slate-500 uppercase tracking-widest text-[10px] font-bold">
                        <th class="px-6 py-4">Klient & Služba</th>
                    ...
                    @forelse($appointments as $appointment)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <form method="POST" action="{{ route('owner.appointments.delete', $appointment) }}" onsubmit="return confirmDelete(event, 'Naozaj odstrániť túto rezerváciu?')">
                                    @csrf @method('DELETE')
